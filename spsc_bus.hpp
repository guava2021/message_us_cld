...
static_assert(sizeof(ControlBlock) == 3 * kCacheLine,
              "ControlBlock must be exactly 3 cache lines");
...
inline SlotHeader* slot_ptr(void* shm_base, uint32_t capacity,
                             uint32_t stride, uint64_t pos) noexcept {
...
public:
    ...
    static SharedBus create(const std::string& name,
                            uint32_t capacity,
    ...
    static SharedBus open(const std::string& name) {
        ...
        auto* cb = static_cast<ControlBlock*>(mem);
        while (cb->magic != kMagic) SPSC_PAUSE();
    ...
    SharedBus(SharedBus&& o) noexcept
        : name_(std::move(o.name_)), mem_(o.mem_), size_(o.size_) {
        o.mem_ = nullptr; o.size_ = 0;
    }
    ...
    ~SharedBus() { if (mem_) munmap(mem_, size_); }
...
private:
    SharedBus(std::string n, void* m, size_t s)
        : name_(std::move(n)), mem_(m), size_(s) {}
...
public:
    ...
    [[nodiscard]] bool try_push(const void* payload, uint32_t size) noexcept {
        if (size > max_msg_) [[unlikely]] return false;
        ...
        if (write_pos_ - read_cache_ >= capacity_) {
            ...
            if (write_pos_ - read_cache_ >= capacity_) return false;
        }
    ...
    void push(const void* payload, uint32_t size) noexcept {
        while (!try_push(payload, size)) SPSC_PAUSE();
    }
...
public:
    ...
    uint32_t try_pop(void* out_buf, uint32_t buf_size) noexcept {
        if (read_pos_ == write_cache_) {
            ...
            if (read_pos_ == write_cache_) return 0;
        }
    ...
    uint32_t pop(void* out_buf, uint32_t buf_size) noexcept {
        uint32_t n;
        while ((n = try_pop(out_buf, buf_size)) == 0) SPSC_PAUSE();
        return n;
    }
